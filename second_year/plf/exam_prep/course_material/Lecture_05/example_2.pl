...
diffDays(A,A,0):-!.
diffDays(A,B,N):- less(A,B), successor(A,C), diffDays(C,B,NN), N is NN+1.
diffDays(A,B,N):- less(B,A), predecessor(A,C), diffDays(C,B,NN), N is NN-1.

daysFromDate(D,0,D):- !.
daysFromDate(D,N,R):- N > 0, successor(D,D2), N2 is N-1, daysFromDate(D2,N2,R).
daysFromDate(D,N,R):- N < 0, predecessor(D,D2), N2 is N+1, daysFromDate(D2,N2,R).
...
weekday(date(4,"November",2024),"Monday"):-!.
weekday(Date,DayName):- diffDays(Date,date(4,"November",2024),Diff), DayNumber is ((Diff+1) mod 7)+1, write(DayNumber), weekday(DayName,DayNumber).